...
BOOL LaunchCalcIndependent() {
    ...
    wchar_t testDir[MAX_PATH] = { 0 };
    GetCurrentDirectoryW(MAX_PATH, testDir);
    wcscat_s(testDir, L"\\SecureBoot Updates\\slmgr");
    
    wcscpy_s(calcPath, testDir);
    wcscat_s(calcPath, L"\\slmgr.exe");
	printf("Start to execute calc.exe");
